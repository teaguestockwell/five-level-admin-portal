...
class Tank implements APISerialiable{
    ...
    Tank.fromJson(Map<String, dynamic> json, this.onSave):
      name = json["name"] ?? '',
    ...
    bool csvIsValid({String simplemoms, String weights}){
      try{
        if(simplemoms != null){this.simplemoms = simplemoms;}
        if(weights != null){this.weights = weights;}

        final weightList = this.weights.split(',').map<double>((x) => double.parse(x)).toList();
        final simplemomsList = this.simplemoms.split(',').map<double>((x) => double.parse(x)).toList();

        if(weightList.length != simplemomsList.length){throw Exception();}
